...
use egui::{text_selection::CCursorRange, Color32, FontId, Key, TextEdit};
...
impl LauncherApp {
    pub fn new(config: Config, apps: Vec<String>, frequency: Frequency) -> Self {
        let fg: Color32 = parse_hex_color(&config.appearance.foreground)
            .map(Into::into)
            .unwrap_or(Color32::WHITE);
        let bg: Color32 = parse_hex_color(&config.appearance.background)
            .map(Into::into)
            .unwrap_or(Color32::BLACK);
        let sel_fg: Color32 = parse_hex_color(&config.appearance.selection_fg)
            .map(Into::into)
            .unwrap_or(Color32::WHITE);
        let sel_bg: Color32 = parse_hex_color(&config.appearance.selection_bg)
            .map(Into::into)
            .unwrap_or(Color32::BLUE);
        let match_hl: Color32 = parse_hex_color(&config.appearance.match_highlight)
            .map(Into::into)
            .unwrap_or(Color32::GREEN);
        let prompt_color: Color32 = parse_hex_color(&config.appearance.prompt_color)
            .map(Into::into)
                        ...
                        && results.len() > 1
                    {
                        ...
                        if self.selected > 0 {
                            if self.selected == self.scroll_offset && self.scroll_offset > 0 {
                                let prev_page_start =
                                    self.scroll_offset.saturating_sub(self.page_size.max(1));
